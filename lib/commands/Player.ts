...
export default class PlayerCommands extends Base {
    ...
    async focusChar(player: string | Player, charId: string, targetId: string, options?: Commands.Player.FocusCharOptions): Promise<CharacterResponse> {
        return this.client.api.call<Record<"char", CharacterResponse>>(ResourceIDs.PLAYER({ id: modelId(player) }), "focusChar", { charId, targetId, ...options })
            .then(r => r.char);
    }
    ...
    async lookupChars(player: string | Player, name: string): Promise<Array<LookupCharacter>> {
        return this.client.api.call<Record<"chars", Array<LookupCharacter>>>(ResourceIDs.PLAYER({ id: modelId(player) }), "lookupChars", { name, extended: true })
            .then(r => r.chars);
    }
    ...
    async mail(player: string | Player, fromChar: string | ControlledCharacter, toCharId: string, options: Commands.Player.MailOptions): Promise<CharacterResponse> {
        return this.client.api.call<Record<"toChar", CharacterResponse>>(ResourceIDs.INBOX({ id: modelId(player) }), "send", { fromCharId: modelId(fromChar), toCharId, ...options })
            .then(r => r.toChar);
    }
    ...
    async unfocusChar(player: string | Player, charId: string, targetId: string, puppeteerId?: string): Promise<CharacterResponse> {
        return this.client.api.call<Record<"char", CharacterResponse>>(ResourceIDs.PLAYER({ id: modelId(player) }), "unfocusChar", { charId, targetId, puppeteerId })
            .then(r => r.char);
    }
