...
class Puppet extends BaseModel implements PuppetProperties {
    ...
    async setSettings(options: Omit<Commands.Player.SetCharSettingsOptions, "puppeteerId">): Promise<null> {
        return this.client.getPlayer().then(player => player.setCharSettings(this.id, { puppeteerId: this.char.id, ...options }));
    }
    ...
    async wakeup(hidden?: boolean): Promise<ControlledCharacter> {
        return this.client.getPlayer().then(player => player.controlPuppet(this.char.id, this.id, true)).then(ctrl => ((ctrl.wakeup(hidden, true), ctrl)));
    }
}
