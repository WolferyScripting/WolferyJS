...
export default function registerCollections(client: WolferyJS, res: ResClient): void {
    ...
    res.registerCollectionType(ResourceIDs.OUTGOING_REQUESTS({ id: "*" }), (api, rid) => new Requests(client, api, rid));
    res.registerCollectionType(ResourceIDs.INCOMING_REQUESTS({ id: "*" }), (api, rid) => new Requests(client, api, rid));
